    ...
    try {
      if (userPw.isEmpty) {
        throw ChangePWModelError(
            message: '현재 비밀번호가 입력되지 않았습니다.', code: "USR-F900");
      }
      if (!passwordRegExp.hasMatch(newPw)) {
        throw ChangePWModelError(
            message: '새로운 비밀번호가 형식에 맞지 않습니다.', code: "USR-F200");
      }
      if (newPw != confirmNewPw) {
        throw ChangePWModelError(message: '비밀번호가 일치하지 않습니다.', code: "USR-F300");
      }
      ...
      state = ChangePWModelError(message: '예외발생 - $e');
